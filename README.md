...
I used the function `set.seed()` to set a random seed. This initiated a specific sequence of random numbers such that the random walks were reproducible each time. When the code is rerun, it produces an identical simulation of random walks every time. In this case I specified both the random walks with identical seeds (`set.seed()`) and hence the right and left graphs showed identical random walks as they were fed identical sequences of numbers. 
